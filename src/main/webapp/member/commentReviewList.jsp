...
List<ReviewCommentBean> commentList = cdao.getMyCommentList(startRow, pageSize, id);
%>
<div id="wrap">
<!-- 헤더 -->
...
<!-- 	본문 -->
	<article>
		...
		<table id="notice">
			...
			%>
				<tr onclick="location.href='../reviewboard/content.jsp?num=<%=cb.getBoardnum() %>'">
		...
		if(startPage > pageBlock) {
			%><a href="commentReviewList.jsp?pageNum=<%=startPage-pageBlock %>">Prev</a><%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="commentReviewList.jsp?pageNum=<%=i%>"><%=i %></a><%
		}
		if(endPage < pageCount) {
			%><a href="commentReviewList.jsp?pageNum=<%=endPage+pageBlock %>">Next</a><%
		}
		%>
		</div>
	</article>
