...
List<BoardBean> boardList = bdao.getMyBoardList(startRow, pageSize, id);
...
<!-- 	본문 -->
	<article>
		...
		<table id="notice">
			...
			%>
				<tr onclick="location.href='../board/content.jsp?num=<%=bb.getNum() %>'">
				<td><%=bb.getNum() %></td>
		...
		int count = bdao.getMyBoardCount(id);
		int pageCount = count / pageSize + (count%pageSize==0 ? 0 : 1);
		if(endPage > pageCount) {
			endPage = pageCount; 
		}
		if(startPage > pageBlock) {
			%><a href="writeList.jsp?pageNum=<%=startPage-pageBlock %>">Prev</a><%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="writeList.jsp?pageNum=<%=i%>"><%=i %></a><%
		}
		if(endPage < pageCount) {
			%><a href="writeList.jsp?pageNum=<%=endPage+pageBlock %>">Next</a><%
		}
		%>
		</div>
	</article>
