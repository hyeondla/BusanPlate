...
<!-- 본문 -->
	...
	List<FileBoardBean> boardList = bdao.fileGetLocationList(startRow, pageSize, location);
	int count = bdao.fileGetLocationCount(location);
	
	ReviewCommentDAO cdao = new ReviewCommentDAO();
	%>
	<article>
		<h1>리뷰게시판 - <%=location %></h1>
		<table id="notice">
...
// 		int count = bdao.fileGetBoardCount(search);
		...
		if(startPage > pageBlock) {
			%><a href="listLocation.jsp?pageNum=<%=startPage-pageBlock %>&location=<%=location %>">Prev</a><%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="listLocation.jsp?pageNum=<%=i%>&location=<%=location %>"><%=i %></a><%
		}
		if(endPage < pageCount) {
			%><a href="listLocation.jsp?pageNum=<%=endPage+pageBlock %>&location=<%=location %>">Next</a><%
		}
		%>
		</div>
	</article>
