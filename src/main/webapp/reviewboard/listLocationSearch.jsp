...
<!-- 본문 -->
	...
	List<FileBoardBean> boardList = bdao.fileGetLocationList(startRow, pageSize, location, search);
	int count = bdao.fileGetLocationCount(location, search);
	
	ReviewCommentDAO cdao = new ReviewCommentDAO();
	%>
	<article>
		<h1>리뷰게시판 - <%=location %></h1>'<%=search %>' 검색 결과입니다 (<%=count %>건)<br><br>
		<table id="notice">
		...
		<div id="table_search">
			<form action="listLocationSearch.jsp" method="post">
...
// 		int count = bdao.fileGetBoardCount(search);
		...
		if(startPage > pageBlock) {
			%><a href="listLocationSearch.jsp?pageNum=<%=startPage-pageBlock %>&location=<%=location %>&search=<%=search %>">Prev</a><%
		}
		for(int i=startPage; i<=endPage; i++) {
			%><a href="listLocationSearch.jsp?pageNum=<%=i%>&location=<%=location %>&search=<%=search %>"><%=i %></a><%
		}
		if(endPage < pageCount) {
			%><a href="listLocationSearch.jsp?pageNum=<%=endPage+pageBlock %>&location=<%=location %>&search=<%=search %>">Next</a><%
		}
		%>
		</div>
	</article>
